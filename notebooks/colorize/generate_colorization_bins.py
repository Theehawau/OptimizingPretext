...
def compute_color_bins_frequency(opts):
    ...
    final_gamut = None
    num_images = len(image_paths['image'])
    ...
    for idx in range(num_images):
        ...
        try:
            ...
            img= image_paths['image'][idx].convert('RGB') 
            img = resize_img(img,HW=(256,256), resample=3)
            img = convert2lab(img)
            img = HWC2CHW(img)[1:, :, :]
            x1, y1 = img[0, :], img[1, :]
...
def main():
    ...
    parser.add_argument('--output_path', type=str, default="datasets/colorize_bins/",
