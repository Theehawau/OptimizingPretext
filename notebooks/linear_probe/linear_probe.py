...
Saves accuracy & classification report at different layers after fitting a LR classifier on top of the features
extracted from a backbone ResNet model. Also stores the extracted features in .npz files for further analysis
...
def run_linear_probe(config):
    ...
    datasets.config.DOWNLOADED_DATASETS_PATH = Path('/l/users/emilio.villa/huggingface/datasets')
    download_path = '/l/users/emilio.villa/huggingface/datasets'
    access_token = None #add here huggingface access token if requested
    
    dataset = load_dataset(hub_names_dict[config['dataset']], token=access_token, cache_dir=download_path)
